...
module.exports = (pool) => {
  ...
  // ── GET /api/backup ── dump all tables to JSON ────────────────────────────
  router.get('/', async (req, res) => {
    try {
      const [employees, routes, routeLogs, packOuts, clockLogs] = await Promise.all([
  ...
  router.post('/restore', async (req, res) => {
    ...
    const { employees, routes, route_logs, pack_out_logs, clock_logs } = backup.tables;
    ...
    try {
      ...
      if (employees?.length) {
        for (const r of employees) {
          await client.query(
            `INSERT INTO employees (id, name, employee_number, position, active, created_at)
             VALUES ($1,$2,$3,$4,$5,$6)
             ON CONFLICT (id) DO UPDATE SET
               name=EXCLUDED.name, employee_number=EXCLUDED.employee_number,
               position=EXCLUDED.position, active=EXCLUDED.active`,
            [r.id, r.name, r.employee_number || null, r.position || null, r.active !== false, r.created_at || new Date()]
          );
        }
      ...
      if (routes?.length) {
        for (const r of routes) {
          await client.query(
            `INSERT INTO routes (id, route_name, description, area, active, created_at)
             VALUES ($1,$2,$3,$4,$5,$6)
             ON CONFLICT (id) DO UPDATE SET
               route_name=EXCLUDED.route_name, description=EXCLUDED.description,
               area=EXCLUDED.area, active=EXCLUDED.active`,
            [r.id, r.route_name, r.description || null, r.area || null, r.active !== false, r.created_at || new Date()]
          );
        }
          ...
          await client.query(
            `INSERT INTO route_logs
               (id, employee_id, log_date, route_number, punch_in, first_stop_time,
                route_complete_time, punch_out, notes, created_at, updated_at)
            ...
            [r.id, r.employee_id, r.log_date, r.route_number || null,
             ...
             r.punch_out || null, r.notes || null, r.created_at || new Date(), r.updated_at || new Date()]
          );
        }
      ...
      if (pack_out_logs?.length) {
        for (const r of pack_out_logs) {
          await client.query(
            `INSERT INTO pack_out_logs (id, route_log_id, seq, pack_out_time, back_on_route_time, created_at)
             VALUES ($1,$2,$3,$4,$5,$6)
             ON CONFLICT (id) DO UPDATE SET
               route_log_id=EXCLUDED.route_log_id, seq=EXCLUDED.seq,
               pack_out_time=EXCLUDED.pack_out_time, back_on_route_time=EXCLUDED.back_on_route_time`,
            [r.id, r.route_log_id, r.seq || 1, r.pack_out_time || null, r.back_on_route_time || null, r.created_at || new Date()]
          );
        }
      ...
      if (clock_logs?.length) {
        for (const r of clock_logs) {
          await client.query(
            `INSERT INTO clock_logs (id, employee_id, log_date, clock_in, clock_out, notes, created_at, updated_at)
  ...
  // ── POST /api/backup/erase ── wipe all operational data ──────────────────
  // Requires admin role. Does NOT touch users table.
