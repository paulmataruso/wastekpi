  ...
  router.post('/restore', async (req, res) => {
    ...
    try {
          ...
          await client.query(
            `INSERT INTO employees
             ...
             ON CONFLICT (id) DO UPDATE SET
               ...
               exclude_from_next_up=EXCLUDED.exclude_from_next_up`,
            [
              r.id,
              r.name,
      ...
      if (route_logs?.length) {
        for (const r of route_logs) {
          await client.query(
            `INSERT INTO route_logs
               (id, employee_id, log_date, route_number,
                punch_in, first_stop_time, route_complete_time,
                to_yard_time, punch_out, notes, created_at, updated_at)
            ...
            [
              r.id, r.employee_id, r.log_date,
              r.route_number || null,
              r.punch_in || null,
              r.first_stop_time || null,
              r.route_complete_time || null,
              r.to_yard_time || null,
              r.punch_out || null,
              r.notes || null,
              r.created_at || new Date(),
        ...
        await client.query(
          `SELECT setval('route_logs_id_seq', $1)`,
          [Math.max(...route_logs.map(r => r.id))]
        );
      }
      ...
      if (pack_out_logs?.length) {
        for (const r of pack_out_logs) {
          await client.query(
            `INSERT INTO pack_out_logs
               (id, route_log_id, seq, pack_out_time, back_on_route_time,
                location, created_at)
             VALUES ($1,$2,$3,$4,$5,$6,$7)
             ON CONFLICT (id) DO UPDATE SET
               route_log_id=EXCLUDED.route_log_id,
               seq=EXCLUDED.seq,
               pack_out_time=EXCLUDED.pack_out_time,
               back_on_route_time=EXCLUDED.back_on_route_time,
               location=EXCLUDED.location`,
            [
              r.id, r.route_log_id,
              r.seq || 1,
              r.pack_out_time || null,
              r.back_on_route_time || null,
              r.location || null,
              r.created_at || new Date(),
            ]
          );
        ...
        await client.query(
          `SELECT setval('pack_out_logs_id_seq', $1)`,
          [Math.max(...pack_out_logs.map(r => r.id))]
        );
      }
      ...
      if (clock_logs?.length) {
        for (const r of clock_logs) {
          await client.query(
            `INSERT INTO clock_logs
               (id, employee_id, log_date, clock_in, clock_out,
        ...
        await client.query(
          `SELECT setval('clock_logs_id_seq', $1)`,
          [Math.max(...clock_logs.map(r => r.id))]
        );
      }
